...
let adminAreaChartInstance = null; // For the admin dashboard area chart
...
/* ============================================================================
   ADMIN DASHBOARD INITIALIZATION & RENDERING
   ============================================================================ */

function initAdminPage() {
  if (typeof requireLogin === "function" && !requireLogin()) return;

  const deptListEl = document.getElementById("adminDeptList");
  if (!deptListEl) return;

  // Build the sidebar from the quizzes that are loaded
  const entries = Object.entries(window.DMI_QUESTION_SETS || {});
  if (!entries.length) {
    deptListEl.innerHTML =
      '<span class="text-muted small px-2">No departments found. Check quiz scripts loading.</span>';
    return;
  }
  ...
  entries.forEach(([code, data], index) => {
    ...
    btn.dataset.deptCode = code;
    btn.textContent = data.shortcode
      ? `${data.shortcode} – ${data.title}`
      : data.title;

    btn.addEventListener("click", () => handleAdminDeptClick(code));
  ...
  handleAdminDeptClick(entries[0][0]);
}

function handleAdminDeptClick(deptCode) {
  const deptData = window.DMI_QUESTION_SETS[deptCode];
  if (!deptData) return;
  ...
  const deptListEl = document.getElementById("adminDeptList");
  if (deptListEl) {
    deptListEl.querySelectorAll(".nav-link").forEach(link => {
      link.classList.toggle(
        "active",
        link.dataset.deptCode === String(deptCode)
      );
    });
  }

  // Load saved answers for this department
  const saved = JSON.parse(localStorage.getItem(getStorageKeyFor(deptCode)) || "{}");
  ...
  const deptTitleEl = document.getElementById("adminDeptTitle");
  const deptSubtitleEl = document.getElementById("adminDeptSubtitle");
  const overallEl = document.getElementById("adminOverallPercent");
  const qCardEl = document.getElementById("adminQuestionsCard");
  const dataStatusEl = document.getElementById("adminDataStatus");
  const bandNameEl = document.getElementById("adminBandName");
  const bandDescEl = document.getElementById("adminBandDesc");

  if (deptTitleEl) deptTitleEl.textContent = deptData.title;
  if (deptSubtitleEl)
    deptSubtitleEl.textContent = `Admin view: ${answered} of ${deptData.totalQuestions} questions answered.`;

  if (overallEl) overallEl.textContent = percent ? `${percent}%` : "0%";
  if (qCardEl) qCardEl.textContent = `${answered} / ${deptData.totalQuestions}`;

  if (dataStatusEl) {
    if (answered === 0) {
      dataStatusEl.textContent = "No responses stored yet.";
    } else if (answered < deptData.totalQuestions) {
      dataStatusEl.textContent = "Partially completed questionnaire.";
    } else {
      dataStatusEl.textContent = "Full questionnaire completed.";
    }
  }
  ...
  // Area-level breakdown using existing helper
  const summary = computeDynamicAreaScores(deptData, saved);
...
function renderAdminAreaList(summary) {
  ...
  // Overall line at the bottom
  const overallLi = document.createElement("li");
  overallLi.className =
    "list-group-item d-flex justify-content-between align-items-center fw-semibold";
  overallLi.innerHTML = `
    <span>Overall</span>
    <span class="badge bg-primary text-white">${summary.overall}%</span>
  `;
